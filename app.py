...
def main():
    ...
    with col2:
        if not guess:
            st.write(f"Attempts Left: 7")
        if guess:
            st.write(f"Attempts Left: {6-st.session_state.attempts}")
    ...
    if guess:
        if st.session_state.attempts < 6:
            ...
            debug.error(
                f"Sorry you lost😔😔 ! Correct guess is {st.session_state.number}"
            )
